...
\subsection{Feature Alignment and Masking}
\label{sec:feature_encoder_module}

The preceding encoder modules operate on heterogeneous inputs: cached
ChemBERTa tokens, RDKit atom descriptors, EGNN conformer tokens, and ProtBERT
residue embeddings. Before the fusion module can compare drug and protein
...
encoders and the interaction module.

First, the active streams are aligned to the shared dimension $d=128$:
\begin{equation}
...
\end{equation}
Here $\psi_{1D}$ is the layer normalization applied to the cached ChemBERTa
...
unmasked mean becomes the ligand context used by FiLM in the protein encoder:
\begin{equation}
  \mathbf{p}_g =
...
protein encoder, and protein-side Mamba refinement. The final BiIntention module
