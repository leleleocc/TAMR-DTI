...
\begin{equation}
  s = f_{\theta}(D,T), \qquad
  \hat{y} = p_{\theta}(y=1\mid D,T)=\sigma(s),
  \label{eq:dti_prediction}
\end{equation}
where $s$ is the prediction logit, $\sigma(\cdot)$ is the sigmoid function, and
$\theta$ denotes all trainable parameters. For a mini-batch
...
matrix $\mathbf{X}^{1D}\in\mathbb{R}^{354\times128}$. For the 2D topological
...
Figure~\ref{fig:tamr_dti_architecture} shows the TAMR-DTI architecture,
including the three drug streams, the target-aware conformer
weighting path, ligand-conditioned protein encoding, protein-side Mamba
refinement, BiIntention-based cross-modal matching, and final interaction
prediction.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{figures/tamr_dti_architecture_v6_imagegen_raw.png}
\caption{TAMR-DTI architecture. The model integrates SMILES
semantics, molecular topology, and target-aware conformer geometry on the drug
side; uses ligand-conditioned FiLM and protein-side Mamba refinement on the
protein side; and performs final drug-target matching through BiIntention before
...
\begin{aligned}
  ...
  \mathbf{h} &= \operatorname{BiIntention}(\mathbf{D},\tilde{\mathbf{P}}),\\
  s &= \operatorname{MLP}(\mathbf{h}), \qquad
  \hat{y}=\sigma(s).
\end{aligned}
\label{eq:overall_pipeline}
\end{equation}
Here $\mathbf{X}^{1D}\in\mathbb{R}^{L_d\times d}$ denotes the ChemBERTa drug
...
sequence before protein-side Mamba refinement, $\tilde{\mathbf{P}}$ is the
refined protein token sequence, $\mathbf{d}_g$ is the global ligand context used
for ligand-conditioned protein encoding, and $\mathbf{h}\in\mathbb{R}^{2d}$ is
...
$\mathbf{X}^{1D}\in\mathbb{R}^{L_d\times d}$. In preprocessing, the
...
\begin{aligned}
  \tilde{\mathbf{X}}^{1D}
  &= \operatorname{LayerNorm}(\mathbf{X}^{1D}),\\
  \tilde{\mathbf{X}}^{geo}
  &= \operatorname{LayerNorm}(\mathbf{X}^{geo}),\\
  \mathbf{G}^{D}
  &= \sigma\!\left(
      \mathbf{W}_{f}
      [\tilde{\mathbf{X}}^{1D};\tilde{\mathbf{X}}^{geo}]
      +\mathbf{b}_{f}
    \right),\\
  \mathbf{D}
  &= \mathbf{G}^{D}\odot\tilde{\mathbf{X}}^{1D}
   + (1-\mathbf{G}^{D})\odot\tilde{\mathbf{X}}^{geo}.
\end{aligned}
\label{eq:drug_gate}
\end{equation}
...
protein tokens. ProtBERT provides residue-level sequence priors, and the FiLM
path lets the paired ligand modulate these tokens before cross-modal fusion.
...
\begin{equation}
  ...
  \tilde{\mathbf{P}}
  =
  \mathbf{P}+\tau(\mathbf{P}^{m}-\mathbf{P}).
  \label{eq:mamba_gate}
\end{equation}
The gate logit is initialized as $\eta=-2.0$, giving $\tau\approx0.119$. The
...
\begin{equation}
  ...
  \mathbf{P}^{s}=\operatorname{SA}_{P}(\tilde{\mathbf{P}}),
...
\begin{equation}
  ...
  \mathbf{h}=[\mathbf{h}_{d};\mathbf{h}_{p}]
  \in\mathbb{R}^{2d}.
...
$\mathbf{h}\in\mathbb{R}^{256}$ to a scalar interaction logit. It is implemented
...
\begin{aligned}
  ...
  \left(
    \operatorname{ReLU}(\mathbf{W}_{1}\mathbf{h}+\mathbf{b}_{1})
  \right),
  &&
...
$\hat{y}_i=\sigma(s_i)$. The mini-batch loss is
\begin{equation}
  \mathcal{L}_{BCE}
  =
  -\frac{1}{B}
